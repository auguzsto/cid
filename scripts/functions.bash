...
Config_smbfile () {
	function Filter_AddCfgFile () {
        ...
        STR='browse?able|comment|create mask|dedicated keytab file|directory( mask)?|disable (netbios|spoolss)|force (create|directory) mode|guest ok|idmap (backend|config.*|gid|uid)|inherit (acls|permissions)|kerberos method|load printers|map (acl inherit|archive|hidden|system)|path|printable|printcap( name)?|printer( name)?|printing|print ok|public|read (list|only)|realm|root preexec|security|server role|smb ports|store dos attributes|template (homedir|shell)|winbind (expand groups|gid|normalize names|nss info|offline logon|refresh tickets|separator|uid|use default domain)|valid users|workgroup|write?able|write (list|ok)'
...
[global]
	...
	if Check_Options 3; then sed -i '/winbind offline logon = Yes/d' "$SMBFILE"; fi
...
Export_LogonScripts () {
	...
	if [ -d "${NETLOGON}/scripts_cid" ]; then
		[ ! -f "${NETLOGON}/scripts_cid/logon.sh" ] && cp -f "${MODELDIR}/scripts_cid/logon.sh" "${NETLOGON}/scripts_cid/logon.sh"

		if [ ! -f "${NETLOGON}/scripts_cid/logon_root.sh" ]; then
			if [ -f "${NETLOGON}/scripts_cid/.logon_(root).sh" ]; then
				cp -f "${NETLOGON}/scripts_cid/.logon_(root).sh" "${NETLOGON}/scripts_cid/logon_root.sh"
			else
				cp -f "${MODELDIR}/scripts_cid/logon_root.sh" "${NETLOGON}/scripts_cid/logon_root.sh"
			fi
		fi

		[ ! -f "${NETLOGON}/scripts_cid/shares.xml" ] && cp -f "${MODELDIR}/scripts_cid/shares.xml" "${NETLOGON}/scripts_cid/shares.xml"
	else
		cp -rf "${MODELDIR}/scripts_cid" "${NETLOGON}"/
	fi
